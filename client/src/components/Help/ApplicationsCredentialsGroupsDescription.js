...
const ApplicationsCredentialsGroupsDescription = () => {
  ...
  const nsiCredentials = [
    // Группы полномочий
    {
      credential: 'GET_ALL_APPS_CREDS_ACTION',
      description: 'Позволяет получить список всех групп полномочий в приложениях ГИД Неман со всеми полями',
      comment: 'Доступно только Главному Администратору ГИД Неман',
    },
    {
      credential: 'GET_APPS_CREDS_ABBRS_ACTION',
      description: 'Позволяет получить список аббревиатур всех групп полномочий ГИД Неман',
      comment: 'Доступно только Главному Администратору ГИД Неман',
    },
    {
      credential: 'MOD_APP_CREDS_GROUP_ACTION',
      description: 'Позволяет добавить/удалить/отредактировать запись в таблице групп полномочий',
      comment: 'Доступно только Главному Администратору ГИД Неман. Удаление группы полномочий влечет удаление всех ссылок на него в таблице ролей',
    },
    {
      credential: 'MOD_APP_CREDENTIAL_ACTION',
      description: 'Позволяет добавить/удалить/отредактировать запись в таблице полномочий группы',
      comment: 'Доступно только Главному Администратору ГИД Неман. Удаление конкретного полномочия группы влечет удаление всех ссылок на него в таблице ролей',
    },
    // Роли
    {
    ...
    },
    {
      ...
      description: 'Позволяет: добавить запись в таблицу ролей, добавить запись в список полномочий роли, удалить роль, отредактировать информацию о роли',
    ...
    },
    {
      ...
      description: 'Позволяет изменить список полномочий для указанной роли',
      comment: 'Доступно только Главному Администратору ГИД Неман',
    },
    // Должности
    {
  ...
  const dy58Credentials = [
    ...
    {
      credential: 'VIEWER',
      description: 'Полномочия пользователя, которому доступно лишь чтение информации: получение списка (текущих) документов',
      comment: '',
    },
  ];
  ...
  return (
    <>
      <Title level={3} id="applications-description">Группы полномочий</Title>
      <p className="help-paragraph">
        На данной странице Главный Администратор системы может вести таблицу групп полномочий пользователей в
        приложениях ГИД Неман.
        ...
        Проверка полномочий пользователей осуществляется серверной частью ГИД Неман при каждом запросе со стороным
        пользователя с любого приложения ГИД Неман.
      </p>
      <p className="help-paragraph">
        Все "известные" серверу ГИД Неман полномочия пользователей можно условно разделить на логические группы:
      </p>
      ...
      <p className="help-paragraph">
        Для более удобного формировании разрешений доступа пользователям к различным функциям ГИД Неман
        Главный Администратор ГИД Неман должен объединить полномочия из одной или нескольких групп полномочий в роли.
      </p>
    </>
  );
};

export default ApplicationsCredentialsGroupsDescription;
