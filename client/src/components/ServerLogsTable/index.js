...
import serverLogsTableColumns from './ServerLogsTableColumns';
...
/**
 * Компонент таблицы с логами действий сервера.
 */
const ServerLogsTable = () => {
  ...
  const columns = serverLogsTableColumns({ getColumnSearchProps });
  ...
  const fetchData = useCallback(async (timeSpan, page, filters) => {
    ...
    try {
      ...
      const res = await request(ServerAPI.GET_SERVER_LOGS_LIST, 'POST',
        {
      ...
      const tableData = res.data.map((item) => getAppServerLogObjFromDBServerLogObj(item));
  ...
  return (
    <>
      <Title level={2} className="center top-margin-05">Логи действий сервера</Title>
      <Form
        layout="inline"
        size="small"
        form={form}
        name="set-server-logs-search-params-form"
        onFinish={onFinish}
      >
        <Form.Item
