...
const ecdTrainSectorBlocksTableColumns = (props) => {
  const {
    isEditing,
    editingKey,
    handleEdit,
      ...
      render: (_, record) => {
        ...
        return editable ? (
          ...
          <Row>
            ...
            <Col>
              <Popconfirm title="Удалить запись?" onConfirm={() => handleDel(record[BLOCK_FIELDS.KEY])}>
                <a
                  href="#!"
                  disabled={editingKey !== '' || (recsBeingProcessed && recsBeingProcessed.includes(record[BLOCK_FIELDS.KEY]))}
                  style={{
...
export default ecdTrainSectorBlocksTableColumns;
