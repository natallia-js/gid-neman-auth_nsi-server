...
/**
 * Компонент таблицы с логами действий администраторов.
 */
const AdminsLogsTable = () => {
  ...
  const fetchData = useCallback(async (timeSpan, page) => {
    ...
    try {
      ...
      const res = await request(ServerAPI.GET_ADMINS_LOGS_LIST, 'POST',
        {
  ...
  return (
    <>
      <Title level={2} className="center top-margin-05">Логи действий администраторов</Title>
      <Form
        layout="inline"
        size="small"
        form={form}
        name="set-admins-logs-search-params-form"
        onFinish={onFinish}
      >
        <Form.Item
...
export default AdminsLogsTable;
