...
const orderRecipientSchema = new Schema({
  // значение поля _id записи - это id адресата (лица из соответствующей коллекции базы данных), если
  // значение поля fio определено; в противном случае, значение поля _id mongodb определяет самостоятельно
...
const orderTextSchema = new Schema({
  // значение поля _id записи - это id соответствующего элемента шаблона;
  // для документа, изданного не на основе шаблона, значение поля _id генерируется автоматически
