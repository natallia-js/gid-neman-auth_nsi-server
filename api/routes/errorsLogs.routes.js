...
/**
 * Обрабатывает запрос на получение списка логов серверных ошибок.
 *
 ...
 */
 router.post(
  ...
  (req, _res, next) => { req.requestedAction = AUTH_NSI_ACTIONS.GET_ERRORS_LOGS; next(); },
  ...
  async (req, res) => {
    ...
    try {
      ...
      const matchFilter = {};
      if (!datetimeEnd) {
        matchFilter.errorTime = { $gte: startDate };
      } else {
        matchFilter.errorTime = { $gte: startDate, $lte: endDate };
      }
      ...
      // Ищем данные
      const data = await ErrorsLog.aggregate(aggregation);
      ...
      addError({
        errorTime: new Date(),
        action: 'Получение списка логов серверных ошибок',
        error: error.message,
        actionParams: {
